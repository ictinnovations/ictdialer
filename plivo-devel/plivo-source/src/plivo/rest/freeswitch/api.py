...
class PlivoRestApi(object):
                ...
                if element.tag == 'Play':
                    ...
                    if sound_file:
                        ...
                        if loop == MAX_LOOPS:
                            break
                # SendFax element
                elif element.tag == 'SendFax':
                    child_instance = elements.SendFax()
                    child_instance.parse_element(element)
                    child_instance.prepare(self._rest_inbound_socket)
                    sound_file = child_instance.sound_file_path
                    if sound_file:
    ...
    def send_fax(self):
        ...
        """
        self._rest_inbound_socket.log.debug("RESTAPI SendFax with %s" \
                                        % str(request.form.items()))
        msg = ""
        result = False
        ...
        # now do the job !
        if self._rest_inbound_socket.send_fax_on_call(calluuid, sounds_list, legs,
                                        length=length, schedule=0, mix=mix, loop=loop):
            msg = "SendFax Request Executed"
            result = True
            return self.send_response(Success=result, Message=msg)
