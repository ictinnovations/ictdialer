...
class SendFax(Element):
    """SendFax local tiff file or at a URL
    ...
    def execute(self, outbound_socket):
        if self.fax_file_path:
            ...
            res = outbound_socket.txfax(fax_path)
            if res.is_success():
                event = outbound_socket.wait_for_action()
                if event.is_empty():
                    outbound_socket.log.warn("SendFax Break (empty event)")
                    return
                outbound_socket.log.debug("SendFax done (%s)" \
                        % str(event['Application-Response']))
            else:
                outbound_socket.log.error("SendFax Failed - %s" \
                                % str(res.get_response()))
            outbound_socket.log.info("SendFax Finished")
            return
        else:
            ...
            for child_instance in self.children:
                if isinstance(child_instance, Play):
                    ...
                    if sound_file:
                        ...
                        if loop == MAX_LOOPS:
                            break
                elif isinstance(child_instance, SendFax):
                    sound_file = child_instance.sound_file_path
                    if sound_file:
                        loop = child_instance.loop_times
                        if loop == 0:
                            loop = MAX_LOOPS  # Add a high number to SendFax infinitely
                        # SendFax the file loop number of times
                        for i in range(loop):
                            self.sound_files.append(sound_file)
                        # Infinite Loop, so ignore other children
                        if loop == MAX_LOOPS:
